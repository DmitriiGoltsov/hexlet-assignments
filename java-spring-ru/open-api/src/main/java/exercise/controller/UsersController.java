...
public class UsersController {
    ...
    // BEGIN
    @Operation(summary = "Update information of the particular User. User is found with his/her id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User with that id is found and updated"),
            @ApiResponse(responseCode = "404", description = "User with that id is not found")
    })
    @PatchMapping(path = "/{id}")
    public User updateUser(
            @Parameter(description = "User's id to be updated")
            @PathVariable long id,
            @Parameter(description = "New User entity that will update the old one")
